...
def _build_lif_node(snn: SNNParams, v_threshold: Optional[float] = None) -> nn.Module:
    ...
    if snn.neuron_type == "lif":
        return neuron.LIFNode(
            step_mode="m", tau=snn.tau, detach_reset=True,
            v_threshold=vth, backend=snn.backend,
        )
    if snn.neuron_type == "plif":
        return neuron.ParametricLIFNode(
            step_mode="m", init_tau=snn.tau, detach_reset=True,
            v_threshold=vth, backend=snn.backend,
        )
    if snn.neuron_type == "nlif":
        return neuron.LIFNode(
            step_mode="m", tau=snn.tau, detach_reset=True,
            v_threshold=vth, backend=snn.backend,
        )
    if snn.neuron_type == "glif":
        return neuron.GatedLIFNode(step_mode="m", backend=snn.backend)
    raise ValueError(
...
class Token_MS_Attention(nn.Module):
    ...
    def __init__(
        ...
        self.p_conv = nn.Conv1d(dim, dim, kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        p = p_conv_out.unsqueeze(2).reshape(T, B, self.num_heads, C // self.num_heads, N)
...
class SpikingTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.attn = Spiking_Self_Attention(
            ...
            ms_scale=ms_scale, attn_drop=attn_drop, proj_drop=drop,
            sr_ratio=sr_ratio, snn=snn,
        )
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = MLP(in_features=dim, hidden_features=mlp_hidden_dim, drop=drop, snn=snn)
...
class PatchEmbeddingStage(nn.Module):
    ...
    def __init__(
        ...
        self.proj3_conv = nn.Conv2d(embed_dims, embed_dims, kernel_size=3, stride=1, padding=1, bias=False)
        ...
        # Residual shortcut
        self.proj_res_conv = nn.Conv2d(embed_dims, embed_dims, kernel_size=1, stride=2, padding=0, bias=False)
        self.proj_res_bn = nn.BatchNorm2d(embed_dims)
        self.proj_res_lif = _build_lif_node(snn)
    ...
    def __init__(
        ...
        stage1 = nn.ModuleList([
            TokenSpikingTransformer(
                ...
                qkv_bias=qkv_bias, ms_scale=ms_scale, drop=drop_rate,
...
def MSFormer_10_512(T: int = 4, snn: SNNParams = None, **kwargs) -> hierarchical_spiking_transformer:
    ...
    embeddings suitable for use as a CLIP image encoder.
