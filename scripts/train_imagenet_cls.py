"""Standalone ImageNet classification training for MSViT (sanity-check script).

Trains MSFormer_10_512 on ImageNet with a standard 1000-class head using
...
Usage:
    ...
    # ImageFolder (separate train/val dirs, pass as --data-dir with a symlink layout
    # or use --train-dir / --val-dir overrides)
    python scripts/train_imagenet_cls.py \\
    ...
    python scripts/train_imagenet_cls.py \\
        ...
        --output-dir ./runs/imagenet_cls_compiled
"""
...
from src.models.visual_encoders.msformer import MSFormer_10_512, SNNParams
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train MSViT on ImageNet classification"
    )
    ...
    parser.add_argument(
        "--precision",
        default="bf16-mixed",
        choices=["32", "16-mixed", "bf16-mixed", "16-true", "16-mixed"],
    )
    # Output
    parser.add_argument("--output-dir", default="./runs/imagenet_cls")
    parser.add_argument(
    ...
    trainer = L.Trainer(
        ...
        callbacks=[
            ...
            ModelCheckpoint(
                ...
                filename="msformer-{epoch:02d}-{val_acc1:.4f}",
                monitor="val_acc1",
                mode="max",
                save_top_k=3,
                save_last=True,
